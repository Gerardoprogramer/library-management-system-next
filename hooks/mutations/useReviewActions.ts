...
export const useReviewActions = (reviewId: string, target: ReviewType, oldRating: number) => {
  const queryClient = useQueryClient();

  const reviewMutation = useMutation({
    mutationFn: (reviewData: editReview) => reviewService.editReview(reviewData, reviewId),
    ...
    onSuccess: (data, variables) => {
      ...
      if (target.type === "book") {
        ...
        queryClient.setQueryData<PageResponse<Review>>(["reviews", "book", bookId], (oldData) => {
          if (!oldData) return oldData;
          return {
            ...oldData,
            content: oldData.content.map((r) => (r.id === reviewId ? { ...r, ...variables } : r)),
