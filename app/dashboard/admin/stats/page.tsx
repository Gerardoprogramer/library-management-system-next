    ...
    <AdminPage title="Estadísticas" description="Indicadores principales del catálogo y su disponibilidad.">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <AdminMetric
          ...
          detail="Títulos publicados en el catálogo"
        />
        <AdminMetric
          icon={PiCheckCircle}
          label="Libros disponibles"
          value={stats.isLoading ? "..." : (stats.data?.totalAvailableBooks ?? 0)}
          detail="Ejemplares con disponibilidad"
          tone="green"
        />
      </div>
      <AdminSection
        title="Lectura del catálogo"
        description="Indicadores para conocer el estado general de tu inventario."
      >
        {stats.isError && (
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
          <div className="rounded-2xl bg-muted/45 p-5">
            ...
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-border">
              <div
                ...
                style={{
                  width: `${stats.data?.totalActiveBooks ? Math.min(100, (stats.data.totalAvailableBooks / stats.data.totalActiveBooks) * 100) : 0}%`,
                }}
              />
            </div>
          </div>
          <div className="rounded-2xl border border-border/70 p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Mantén el catálogo equilibrado para que los usuarios encuentren ejemplares disponibles.
            </p>
          </div>
        </div>
      </AdminSection>
