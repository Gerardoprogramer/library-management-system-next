...
export const LoanCard = ({ loan }: Props) => {
  ...
  return (
    <Card
      className={`overflow-hidden transition-all duration-300 border-white/5 hover:border-white/10 ${loan.overdue ? "border-destructive/40 bg-destructive/5" : "bg-card"}`}
    >
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-1.5 text-muted-foreground/70 mb-4 cursor-help group w-full max-w-60">
                        ...
                        <span className="text-xs truncate block w-full">{loan.notes}</span>
                      </div>
                    </TooltipTrigger>
